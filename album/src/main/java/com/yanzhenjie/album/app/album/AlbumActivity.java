...
public class AlbumActivity extends BaseActivity implements
    ...
    protected void onPermissionGranted(int code) {
        ...
        Runnable mediaRunner = () -> {
            ...
            if (checkedList != null && !checkedList.isEmpty()) {
                ...
                for (AlbumFile checkAlbumFile : checkedList) {
            ...
            mediaHandler.post(() -> {
                ...
                if (mAlbumFolders.get(0).getAlbumFiles().isEmpty()) {
                    Intent intent = new Intent(AlbumActivity.this, NullActivity.class);
        ...
        mediaExecutor.submit(mediaRunner);
    }
                ...
                public void onActivityResult(ActivityResult result) {
                    if (result.getResultCode() == RESULT_OK) {
                        if (result.getData() != null) {
                            String imagePath = NullActivity.parsePath(result.getData());
    ...
    private Action<String> mCameraAction = new Action<String>() {
        ...
        public void onAction(@NonNull String result) {
            ...
            ExecutorService pathConvertExecutor = Executors.newSingleThreadExecutor();
            Handler pathConvertHandler = new Handler(Looper.getMainLooper());

            pathConvertExecutor.execute(() -> {
                AlbumFile albumFile = conversion.convert(result);
                pathConvertHandler.post(() -> {
                    albumFile.setChecked(!albumFile.isDisable());
                    if (albumFile.isDisable()) {
                        if (mFilterVisibility) addFileToList(albumFile);
                        else mView.toast(getResources().getString(R.string.album_take_file_unavailable));
                    } else {
...
//                mView.notifyItem(position);
    ...
    private void callbackResult() {
        ...
        ThumbnailBuilder mThumbnailBuilder = new ThumbnailBuilder(AlbumActivity.this);
        ExecutorService thumbNailExecutor = Executors.newSingleThreadExecutor();
        Handler thumbNailHandler = new Handler(Looper.getMainLooper());
        thumbNailExecutor.execute(() -> {
            for (AlbumFile albumFile : mCheckedList) {
